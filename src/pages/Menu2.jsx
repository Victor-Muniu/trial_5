...
function Menu2() {
  ...
  const clubItems = data.filter(item => item.point_of_sale === 'Bar');
  ...
  const handleSubmitOrder = async (type) => {
    ...
    try {
      const response = await axios.post('https://hotel-backend-zrv3.onrender.com/clubOrders', orderData);
